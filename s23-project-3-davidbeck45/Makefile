...
FLAGS_debug:=-O0 -ggdb3 -DDEBUG_BUILD -fsanitize=address
CFLAGS:=-std=gnu17 -Werror -Wall -Wstrict-prototypes -Wmissing-prototypes \
	-Wmissing-declarations -Wundef -Iinclude
OUTDIR:=build
LD:=$(CC)
LDFLAGS:=$(CFLAGS)

BANNED_FUNCTIONS:=system popen strcat strcpy sprintf \
	execv execve exevp execvpe execl execlp execle fork
CFLAGS_FOR_BANNED_FUNCTIONS:=$(foreach f,$(BANNED_FUNCTIONS),-D$(f)=__the_function_$(f)_is_banned_do_not_use_it)
...
cmd_c_to_o_name = CC
cmd_c_to_o = $(CC) $(CFLAGS) $(CFLAGS_FOR_BANNED_FUNCTIONS) $(target_flags) $(DEPFLAGS) -c $< -o $@
